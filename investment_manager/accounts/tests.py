...
class UserPermissionsTests(BaseTestCase):
    ...
    def setUp(self):
        ...
        # investment accounts for different permission levels
        ...
        self.transaction1 = Transaction.objects.create(
            ...
            amount=100, 
            timestamp=timezone.make_aware(timezone.datetime(2024, 1, 15))
        )
        self.transaction2 = Transaction.objects.create(
            user=self.user, 
            account=self.account3, 
            amount=200, 
            timestamp=timezone.make_aware(timezone.datetime(2024, 1, 10))
        )
        self.transaction3 = Transaction.objects.create(
            user=self.user, 
            account=self.account2, 
            amount=300, 
            timestamp=timezone.make_aware(timezone.datetime(2024, 1, 1))
        )
        self.transaction4 = Transaction.objects.create(
            user=self.user, 
            account=self.account2, 
            amount=400, 
            timestamp=timezone.make_aware(timezone.datetime(2024, 2, 1))
        )
    ...
    def test_user_with_crud_permission_can_create_transaction(self):
        self.authenticate_user('user', 'password')
        response = self.client.post('/api/transactions/', {'account': self.account2.id, 'amount': 500, 'timestamp': timezone.now()})
    ...
    def test_user_with_post_only_permission_can_create_transaction(self):
        self.authenticate_user('user', 'password')
        response = self.client.post('/api/transactions/', {'account': self.account3.id, 'amount': 300, 'timestamp': timezone.now()})
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_user_with_crud_permission_can_read_transaction(self):
        self.authenticate_user('user', 'password')
        response = self.client.get(f'/api/transactions/{self.transaction1.id}/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_user_with_crud_permission_can_update_transaction(self):
        self.authenticate_user('user', 'password')
        response = self.client.patch(f'/api/transactions/{self.transaction1.id}/', {'amount': 600})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(float(response.data['amount']), 600.00)

    def test_user_with_crud_permission_can_delete_transaction(self):
        self.authenticate_user('user', 'password')
    ...
    def test_admin_can_filter_transactions_by_date(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertIn('transactions', response.data)
        self.assertEqual(len(response.data['transactions']), 3)  # Expecting 3 transactions
        self.assertEqual(response.data['total_balance'], 600)
